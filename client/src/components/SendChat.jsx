...
const SendChat = ({ socket, setMessages, messages, setNewmsg }) => {
    ...
    const handleOnEnter = async () => {
        try {
            ...
            const { data } = await axios.post(`/message/sendmessage/${id}`, {
                chatid: currentChat._id, content: emoji
            }, {
                headers: {
    ...
    return (<section className="send-chat">
        <InputEmoji
            value={emoji}
            onChange={setEmoji}
            cleanOnEnter
        ...
        />
        <button className="btn chat__btn" onClick={handleOnEnter}>Send</button>
    </section>
